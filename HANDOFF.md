...
**v5: 240 items, each on its own expression** (rebuilt 2026-09-13). Seed 2026, reproducible
...
hold 60 items each. Every trace is 6 steps with exactly one expert-illegal move. Every learner
answer differs from the correct answer. Numbers shown span 1 to 930: no negatives, decimals or
zeros. No foil is ever probed on a trace generated by that same rule. In every A item the error
...
(9 × 2 + 4) - 1 + 8 ÷ 2 + 11            3 × 4 ÷ 2 + 3 × 6 ÷ 3 - 1
= (9 × 6) - 1 + 8 ÷ 2 + 11   <- 2+4     = 12 ÷ 2 + 3 × 6 ÷ 3 - 1
= (9 × 6) - 1 + 4 + 11                  = 6 + 3 × 6 ÷ 3 - 1
...                                     = 9 × 6 ÷ 3 - 1             <- 6+3
```
...
1. **6 operators was forced by the matched pairs; since v5 it is a choice.** Over 2500 bracketed
   expressions, the number supporting BOTH step 1 and step 3 for `outside_bracket_first` is **0 at 4
   ops, 0 at 5 ops, 32 at 6 ops**, so v4's pairs needed 6. v5 needs only one position per
   expression. At 4 ops that rule never reaches step 3 at all, so never go below 5; whether 5 ops
   fills every v5 cell is untested. The pool stays at 6.
2. **Position is SELECTED, not constructed.** Nothing places the error. A misconception is a
   ...
   when. One expression plus one misconception yields many valid traces (75 for
   `4 + 9 × 4 ÷ 2 × (3 + 10) × 2` under add_before_mul), with the single error landing anywhere from
   step 1 to step 5. The builder filters for step 1 and step 3. Left unconstrained, position is
...
3. **`foil_status` is RECORDED but NOT BALANCED.** Refutation (refuted vs unsupported foil) is not a
   factor in this experiment. Pool-wide (v5) it is 58 refuted / 62 unsupported, but per foil it is
   lopsided: `add_before_mul` 14/6, `sub_before_div` 5/15, `same_priority_rtl` 13/7.
...
5. **Most of the evidence sits in two or three of the 6 steps.** Counting how many of the 22
   hypotheses each step eliminates, summed over all 240 v5 traces: step 1 2532, step 2 485, step 3
   1143, step 4 90, step 5 81, step 6 0. Steps 1 and 3 (the error positions) carry 85%; the last step
   carries none because it is forced. Participants read six lines of which roughly two and a half
   matter.
...
- B items: P(agree) 0.000; marginal min 0.000 / mean 0.138 / max 0.333, over only 11 distinct
  values.
- **Error position does nothing to the observer's answers.** Every item is answered correctly at
  both positions and A sits at 1.000 at both. B marginals average 0.123 at step 1 and 0.152 at step
...
⚠️ **Use `probed_marginal` as the observer's response, never `map_profile`.** On 35 of 240 v5 items
(15%) the MAP names TWO rules for a one-misconception item, always with `outside_bracket_first` as
...
Over the 1200 (trace, absent rule) combinations of the v5 pool, **42 (3.5%) give an ABSENT rule a
marginal above 0.35 and 27 (2.2%) above 0.5**, i.e. the trace positively favours a rule the student
does not hold. Max **0.871** (item B145: the trace contains add_before_div, yet P(outside() | trace)
= 0.871). **All 27 over-0.5 cases are `outside_bracket_first`**, as are 40 of the 42 over 0.35. Because
